...
- 本轮验收范围只计 `zb` 招标公告；其他阶段代码保留作兼容回归，不作为本轮省份通过依据。
...
必须看到 `SELF_TEST 26/26 passed`（以 scripts/self-test.cjs 实际用例数为准）。失败时先修门禁，不开始批量联网。
...
| 贵州 | `仁怀` | 仁怀市（1/1） | REST |
| 浙江 | `义乌` | 义乌市（1/1） | EPoint |
| 山东 | `青岛` | 青岛（1/1） | HTML |
| 湖南 | `郴州` | 郴州市（1/1） | bespoke |
| 湖北 | `襄阳` | 襄阳市（1/1） | REST |
| 福建 | `闽侯` | 闽侯县（1/1） | REST |
| 四川 | `大邑` | 大邑县（1/1） | EPoint |
