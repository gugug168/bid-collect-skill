...
| 河南 bespoke `xiaqucode` | `henanNoticeList`，体 `xiaqucode: ad.xiaqucode \|\| "4100"` | ❌ 地市码实测返回 0（仅 `4100` 全省可用） | 不声明逐地市循环 |
...
3. **河南**逐地市标准行政区码实测返回 0，不能声明服务端城市循环；继续使用 `4100` 全省结果和客户端 `--city` 过滤。
...
- `resolveCityTargets(ad, args)`：内部城市目标解析已单测；当前无 adapter 声明经官方验证的 `cityCodes`，实际运行恒返回 null。
- `henanNoticeList` 重构出 `henanFetchPage`（单页抓取），默认行为不变（仍 4100 全省 + crawlRound 翻页）。
- `resolveCityTargets` 仅保留为内部扩展点；没有已验证 `cityCodes` 前不暴露无效的 `--all-cities`/`-A` 公共参数。
- 以上均为**行为守恒**修改：Henan 默认采集与原先完全一致，客户端 `--city` 过滤继续有效。
- 验证：当前完整 checkout 已运行 `node --check` 与 `scripts/self-test.cjs`；最终通过数量以本 PR 最新 CI 为准。

## 七、客户端城市级深度（地级市→区/县安全归一 · 2026-08-16）

在 ① 结论（城市入口 = `--city` 客户端过滤）基础上，进一步补全"城市级深度"：省级平台常只标区县（如「香洲区」），
导致 `--city 珠海` 命中不了——本段让地级市筛词在证据不冲突时扩展到其区县。

- `PREFECTURE_DISTRICTS`：**341 地级市 → 区县** 静态种子表（覆盖 31 省 + 4 直辖市；不含港澳台/新疆兵团）。当前仓库没有可重复生成器或锁定的数据集版本，因此只将其作为待审静态输入，不声称具备可复现构建链。
- 重名区县只在标题/地区字段直接出现地级市名时命中；只有全国唯一归属的区县别名参与地级市扩展，避免“城区”“市中区”等跨城市误匹配。
- `matchesCityFilter` 增强：
  - 正向：`--city 珠海` 除命中标题含「珠海」的记录外，也命中仅标「香洲区/金湾区/斗门区」的记录；
  - 区县筛词不反向放大为整个地级市：`--city 林州` 不会放行仅写“安阳市”的记录。
- 自治州/盟已剥民族前缀，key 取用户实际输入的短名（如 `黔西南布依族苗族自治州`→`黔西南`、`兴安盟`→`兴安`），保证正向查找命中。
- 仅做保守增量匹配；重名或缺少上下文时不猜归属。`--city` 缺省 / `全省` 行为不变（仍返回 true）。
- 验证覆盖正向命中、区县筛词不放大、`市辖区`、`城区`、`市中区` 跨城市负例；最终通过数量以本 PR 最新 CI 为准。
